...
export function ColumnChart({ result, onDrill }: Props) {
  ...
  const values = members.map((m) => {
    ...
    return {
      key: m.key,
      label: m.label,
      value: row?.suppressed ? null : (row?.values[measureKey] ?? 0),
      n: row?.n ?? 0,
      suppressed: row?.suppressed ?? false,
    };
  });
  const max = Math.max(1, ...values.map((v) => (typeof v.value === "number" ? v.value : 0)));
  const suffix = (result.measures[0]?.format as { suffix?: string })?.suffix ?? "";
        ...
        {values.map((v, i) => {
          ...
          return (
            <button
              ...
              title={v.suppressed ? "Disembunyikan (n kecil)" : undefined}
            >
              <span className="text-xs font-semibold tabular-nums text-muted-foreground">
                {v.suppressed ? "—" : `${v.value}${suffix}`}
              </span>
              <div
                className="w-full max-w-[56px] rounded-t-md transition-opacity group-hover:opacity-90"
                style={{
                  height: h,
                  background: v.suppressed
              ...
              <div className="text-[10px] tabular-nums text-muted-foreground">{v.n} org</div>
            </button>
          );
        })}
      </div>

...
export function useOrgChart(initialData: EmployeeNode[]) {
  ...
  const updateTree = (tree: EmployeeNode[], updated: EmployeeNode): EmployeeNode[] => {
  ...
  const addChildNode = (parentId: string, newNode: EmployeeNode): EmployeeNode[] => {
    return data.map((node) =>
      node.id === parentId
        ? { ...node, children: [...(node.children || []), newNode] }
        : {
            ...node,
            children: node.children ? addChildNodeRecursive(node.children, parentId, newNode) : []
          }
    );
  };

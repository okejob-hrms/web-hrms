    ...
    <div className="bg-white rounded-xl border border-gray-200 p-8 md:p-12 shadow-sm max-w-4xl mx-auto">
      <div className="mb-10">
        <h2 className="text-xl font-bold text-slate-900">
          {t('completeOffboardingTasks')}
        </h2>
        <p className="text-slate-500 text-sm mt-1">
          {t('completeOffboardingTasksDesc')}
        </p>
      </div>
      ...
      <div className="relative">
        ...
        <div className="space-y-10">
          {offboardingProgress?.map((task) => {
            ...
            const isHandover =
              task.type === 'work_handover' || task.type === 'document_handover';

            const shouldShowButton = !isEquipment && !(isExitForm && isCompleted);

            const buttonLabel =
              isHandover && isCompleted ? tCommon('edit') : t('start');
                  ...
                  {shouldShowButton && (
                    <Button
                      ...
                      className={cn(
                        ...
                        !isCompleted
                          ? 'bg-[#336192] hover:bg-[#264a70] text-white'
                          : 'border-[#336192] text-[#336192] hover:bg-slate-50',
                      )}

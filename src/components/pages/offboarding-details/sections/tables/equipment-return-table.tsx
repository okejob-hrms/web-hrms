...
export const FormModal = React.memo(function FormModal({
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(t("equipmentReturnCreated"));
      form.reset();
      onOpenChange(false);
      queryClient.invalidateQueries({ queryKey: ["equipment-handover"] });
    },
    onError: (error: any) => {
      toast.error(error.message || t("equipmentReturnCreateFailed"));
    },
  });
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(t("equipmentReturnUpdated"));
      form.reset();
      onOpenChange(false);
      queryClient.invalidateQueries({ queryKey: ["equipment-handover"] });
    },
    onError: (error: any) => {
      toast.error(error.message || t("equipmentReturnUpdateFailed"));
    },
  });
      ...
      <DialogContent className="bg-white md:min-w-5xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-lg sm:text-xl">
            {isEditMode ? t("editEquipmentReturn") : t("addEquipmentReturn")}
          </DialogTitle>
        </DialogHeader>
        <Form {...form}>
          <form
          ...
          >
            <InputForm label={t("workEquipment")} required name="name" />
...
export const EquipmentReturnTable = React.memo(function EquipmentReturnTable({
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(t("equipmentReturnDeleted"));
      setDeleteDialogOpen(false);
      setSelectedItem(null);
      queryClient.invalidateQueries({ queryKey: ["equipment-handover"] });
    },
    onError: (error: any) => {
      toast.error(error.message || t("equipmentReturnDeleteFailed"));
    },
  });
